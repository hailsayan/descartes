...
func NewSurveyHandler(survey *Survey) *SurveyHandler {
	return &SurveyHandler{Survey: survey}
}
...
func (h *SurveyHandler) Flights(c *gin.Context) {
	req := new(FlightRequest)
	...
	err = h.Survey.AddFlight(req.Name)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"Message": err.Error(),
		})
...
func (h *SurveyHandler) Tickets(c *gin.Context) {
	...
	err = h.Survey.AddTicket(req.FlightName, req.PassengerName)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"Message": err.Error(),
		})
...
func (h *SurveyHandler) Comments(c *gin.Context) {
	...
	err = h.Survey.AddComment(req.FlightName, req.PassengerName, Comment{
		Score: req.Score,
		Text:  req.Text,
	})
	if err != nil {
...
func (h *SurveyHandler) GetFlightComments(c *gin.Context) {
	flightName := c.Param("flight")
	hasAverage := c.Query("average")
	if hasAverage == "true" {
		average, err := h.Survey.GetCommentsAverage(flightName)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"Message": err.Error(),
			})
		...
		comments, err := h.Survey.GetComments(flightName)
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"Message": err.Error(),
			})
...
func (h *SurveyHandler) GetComments(c *gin.Context) {
	hasAverage := c.Query("average")
	if hasAverage == "true" {
		averages := h.Survey.GetAllCommentsAverage()
		c.JSON(http.StatusOK, gin.H{
			"Averages": averages,
			"Message":  "OK",
		})
	} else {
		comments := h.Survey.GetAllComments()
		c.JSON(http.StatusOK, gin.H{
			"Message": "OK",
			"Texts":   comments,
		})
...
func (server *Server) Start() {
	engine := gin.Default()
	h := NewSurveyHandler(NewSurvey())
	engine.GET("/", HealthHandler)
	engine.POST("/flights/", h.Flights)
	engine.POST("/tickets/", h.Tickets)
	engine.POST("/comments/", h.Comments)
	engine.GET("/comments/:flight", h.GetFlightComments)
	engine.GET("/comments/", h.GetComments)
	go func() {
		err := engine.Run(fmt.Sprintf(":%d", server.portNumber))
		if err != nil {
			return
		}
	}()
